...
def has_divisor_smaller_than(n, i):
    """checks if there is a divisor for n that smaller than i
...
def divisors(n):
    ...
    """
    if n != 0:
        divitors_list =  list_of_divisor_smaller_than(n, n, divisor_list=[])
        divitors_list.reverse()
        return divitors_list
    else:
        return []
...
def print_binary_sequences_with_prefix(prefix, n):
    """
    prints strings with all the possibilities in len 'n' and starts in prefix
    condition.
...
def print_sequences_helper(prefix, char_list, n):
    ...
    char_lists that starts with prefix condition.
...
def print_no_repetition_sequences_list_helper(sequences_list, prefix,
                                              char_list, n):
    """
    ...
    if len(prefix) == n:
        ...
        for i in range(len(char_list)):
            if str(char_list[i]) not in prefix:
                print_no_repetition_sequences_list_helper(
...
def no_repetition_sequences_list(char_list, n):
    """
    calls print_no_repetition_sequences_list_helper function.
    :param char_list: a list of chars
    :param n:  integer - len of number of possibility
    ...
    if n > 0:
        sequences_list = []
        prefix = ''
        return print_no_repetition_sequences_list_helper(sequences_list, prefix,
                                          char_list, n)
    else:
        return ['']
