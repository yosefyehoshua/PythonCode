...
class Ship:
    ...
    def __repr__(self):
        ...
        """
        ship_tuple = (self.__coordinates, self.__damage_ship_cells,
                      ship_helper.direction_repr_str(Direction, self.__temp_direction), self.__board_size)
    ...
    def move(self):
        ...
        if not self.hit(self.__pos):
            if self.__temp_direction == Direction.UP:
                ...
                if self.__pos[0] + self.__length != self.__board_size - 1:
                ...
                if self.__pos[1] + self.__length != self.__board_size - 1:
    ...
    def hit(self, pos):
        ...
        if pos not in self.__damage_cells:
            ...
            if self.__contains__(pos) is True and pos not in \
                    self.__damage_ship_cells:
    ...
    def coordinates(self):
        ...
        """
        pos_list = []
        if self.__direction is Direction.UP or Direction.DOWN:
            for i in range(self.__length):
                vertical_pos = (self.__pos[0], self.__pos[1] + i)
                pos_list.append(vertical_pos)

        elif self.__direction is Direction.LEFT or Direction.RIGHT:
            for i in range(self.__length):
